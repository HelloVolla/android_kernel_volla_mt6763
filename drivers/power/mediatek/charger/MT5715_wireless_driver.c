...
static int MT5715_handle(u16 flag)
{
	...
	if(irq_flag.ptr[0] & 0x02){//bit 1
		...
		//pr_err("%s: MT5715_handle before INT_LDO_ON  fast_sv \n" , __func__);
		//fast_sv(9000);
		//pr_err("%s: MT5715_handle after INT_LDO_ON  fast_sv \n" , __func__);
		//MT5715_write_buffer(mte, 0x007A, val.ptr, 2);  //update RPP value for MT5715
		//MT5715_write_buffer(mte, 0x0078, val.ptr, 2);  //update RPP value for MT5715
	}
