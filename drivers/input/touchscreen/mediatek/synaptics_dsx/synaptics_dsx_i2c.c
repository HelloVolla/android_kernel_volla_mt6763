...
printk("%s\n",__func__);
	...
	#if defined(CONFIG_PRIZE_HARDWARE_INFO)
    //rmi4_data->firmware_id
	sprintf(current_tp_info.chip,"%s", rmi4_data->rmi4_mod_info.product_id_string);//rmi4_data->firmware_id
    sprintf(current_tp_info.id,"%s","TD4100");
...
static struct i2c_driver tpd_i2c_driver = {
	...
	.driver = {
		   .name = "synaptics_dsx",
		   .owner = THIS_MODULE,
		   .of_match_table = of_match_ptr(synaptics_i2c_dt_match),
     },
