...
#define GT1XX_GESENABLE_PROC_NAME         "gt9xx_enable"
#define GT1XX_GESVAL_PROC_NAME            "gt9xx_gesval"
#pragma pack(1)
typedef struct {
...
#if GTP_GESTURE_WAKEUP
static struct proc_dir_entry *gesture_node_proc_entry = NULL;
#endif
...
#define GT9XX_GESVAL_VALUE_DEFAULT			1

/*d.j add 2015.03.09*/
//static u8 gesenable=1;
u8 gesenable_from_user = GT9XX_GESVAL_VALUE_DEFAULT;
static u8 gesval = 0x00;
/* Node: gt9xx_enable --->flag for gesture ON or OFF */
static ssize_t gt1xx_gesenable_read_proc(struct file *, char __user *, size_t, loff_t *);
static ssize_t gt1xx_gesenable_write_proc(struct file *, const char __user *, size_t, loff_t *);

static struct proc_dir_entry *pgt1xx_gesenable_proc = NULL;
static const struct file_operations gesenable_proc_ops = {
    .owner = THIS_MODULE,
    .read = gt1xx_gesenable_read_proc,
    .write = gt1xx_gesenable_write_proc,
};


/* Node: gt9xx_gesture--->save the value for gesture*/
static ssize_t gt1xx_gesval_read_proc(struct file *, char __user *, size_t, loff_t *);
static ssize_t gt1xx_gesval_write_proc(struct file *, const char __user *, size_t, loff_t *);

static struct proc_dir_entry *pgt1xx_gesval_proc = NULL;
static const struct file_operations gesval_proc_ops = {
    .owner = THIS_MODULE,
    .read = gt1xx_gesval_read_proc,
    .write = gt1xx_gesval_write_proc,
};
static ssize_t gt1xx_gesenable_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    char *ptr = page;    
    GTP_DEBUG("func:%s,line:%d\n",__func__, __LINE__);      
    if (*ppos)
    {
        return 0;
    }    
    ptr += sprintf(ptr, "0x%02x", gesenable_from_user);
    ptr += sprintf(ptr, "\n");
    *ppos += ptr - page;
    return (ptr - page);
}

static ssize_t gt1xx_gesenable_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
	  int i;
    s32 ret = 0;
    char char_gesen[3]={0};
    unsigned long enable;
    GTP_DEBUG("%s,%d write count=%d\n",__func__, __LINE__, (int)count);
    if(count>3)
    {
    		count = 3;
    }
    ...
    GTP_DEBUG("print hex data gt9xx_enable=0x%x\n", (u8)enable); 
    gesenable_from_user = (u8)enable;
	printk("func:%s,line:%d, gesture_enabled:%d\n",__func__, __LINE__, gesture_enabled);
	  return count; 
}
...
/* Node: gt9xx_gesture--->save the value for gesture*/
static ssize_t gt1xx_gesval_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	  char *ptr = page;
	  if (*ppos)
    {
    ...
    GTP_DEBUG("func:%s,line:%d gesval:%d\n",__func__, __LINE__,gesval);    
...
static ssize_t gt1xx_gesval_write_proc(struct file *filp, const char __user *buffer, size_t count, loff_t *off)
{
	  int i;
    s32 ret = 0;
    ...
    GTP_DEBUG("print hex data val=0x%x\n", (u8)val);    
    gesval = (u8)val;
    GTP_DEBUG("print hex data gesval=0x%x\n", gesval);
...
s32 gesture_event_handler(struct input_dev * dev)
{
	...
	gesval = ges_type;	
	input_report_key(dev, KEY_GESTURE_EVENT, 1);
    input_sync(dev);
    input_report_key(dev, KEY_GESTURE_EVENT, 0);
    input_sync(dev);
...
s32 gt1x_init_node(void)
{
	...
	gesture_node_proc_entry = proc_create(GESTURE_NODE, 0666, NULL, &gt1x_fops);
	if (gesture_node_proc_entry == NULL) {
		GTP_ERROR("CAN't create proc entry /proc/%s.", GESTURE_NODE);
		return -1;
	} else {
		GTP_INFO("Created proc entry /proc/%s.", GESTURE_NODE);
	}
	pgt1xx_gesenable_proc = proc_create(GT1XX_GESENABLE_PROC_NAME, 0666, NULL, &gesenable_proc_ops);
    if ( NULL == pgt1xx_gesenable_proc)
    {
        GTP_ERROR("create_proc_entry %s failed\n", GT1XX_GESENABLE_PROC_NAME);
    }
    else
    {
        GTP_DEBUG("create proc entry %s success", GT1XX_GESENABLE_PROC_NAME);
    }    
    pgt1xx_gesval_proc = proc_create(GT1XX_GESVAL_PROC_NAME, 0666, NULL, &gesval_proc_ops);
    if ( NULL == pgt1xx_gesval_proc)
    {
        GTP_ERROR("create_proc_entry %s failed\n", GT1XX_GESVAL_PROC_NAME);
    }
    else
    {
        GTP_DEBUG("create proc entry %s success", GT1XX_GESVAL_PROC_NAME);
    }
#endif
...
#if GTP_HOTKNOT
	if (hotknot_misc_device.this_device != NULL){
