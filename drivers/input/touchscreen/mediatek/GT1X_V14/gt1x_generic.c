...
s32 gt1x_read_version(struct gt1x_version_info * ver_info)
{
	...
	ret = gt1x_i2c_read(GTP_REG_CONFIG_DATA, buf_cfg, 1);

	GTP_INFO("IC VERSION:GT%s_%06X(Patch)_%04X(Mask)_%02X(SensorID)", product_id, patch_id, mask_id >> 8, sensor_id);
	
	/*if(strcmp("5668",product_id) != 0){
		GTP_ERROR("chip  id failed");
		return -1;
	}*/
	#if defined(CONFIG_PRIZE_HARDWARE_INFO)
		sprintf(current_tp_info.chip, "GT%s\n cfg:0x%02x", product_id, buf_cfg[0]);
...
static int gt1x_smart_cover_update_state(void)
{
	...
	if(!dev->suspended) {
		if(dev->state) {  /* near */
		    //add by tangcong for 8802 Glove Mode show 20200724 start
			memset(current_tp_info.more,0,sizeof(current_tp_info.more));
			sprintf(current_tp_info.more,"Glove Mode:on");
	    ...
	    #endif
			memset(current_tp_info.more,0,sizeof(current_tp_info.more));
			sprintf(current_tp_info.more,"Glove Mode:off");
			//add by tangcong for 8802 Glove Mode show 20200724 end
		}
...
static int gt1x_smart_cover_init(void)
{
    ...
    //add by tangcong ,for creat common node  /sys/kernel/prize/smartcover/common_node/state 20200724
    ...
    if (!gt1x_sc_dev->kobj)
		...
		err = sysfs_create_link(gt1x_sc_dev->kobj,&gt1x_i2c_client->dev.kobj,"common_node");
		if (err){
			printk("prize gt1x sysfs_create_link fail\n");
		}
	if(sysfs_create_file(&gt1x_i2c_client->dev.kobj, &sc_attr.attr)) {
		err = -4;
        goto exit_put_kobj;
	}
	#else
...
int gt1x_suspend(void)
{
	...
	gesture_enabled = gesenable_from_user;
	if (gesture_enabled) {
		gesture_enter_doze();
		gt1x_irq_enable();
		gt1x_halt = 0;
