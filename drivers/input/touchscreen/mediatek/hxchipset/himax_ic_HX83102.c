...
static bool hx83102_chip_detect(void)
{
	...
	int ret = 0;
	int i = 0;
	printk("--------1---1----hx83102_chip_detect-----------------------");
	...
	hx83102_pin_reset();
#endif
	printk("--------2---1----hx83102_chip_detect-----------------------");

	ret = kp_himax_bus_read(0x13, tmp_data, 1, HIMAX_I2C_RETRY_TIMES);
	printk("--------2---2----hx83102_chip_detect-----------------------ret=%d",ret);
	if (ret < 0) {
			printk("--------3---2----hx83102_chip_detect-----------------------");
		E("%s: i2c access fail!\n", __func__);
		return false;
	}
		printk("-------4-------hx83102_chip_detect-----------------------");
	if (hx83102_sense_off(false) == false) {
		ret_data = false;
		E("%s:hx83102_sense_off Fail:\n", __func__);
		return ret_data;
	}
	printk("--------5-------hx83102_chip_detect-----------------------");
	...
	for (i = 0; i < 5; i++) {
		...
		if (ret != 0) {
			...
			printk("--------7-------hx83102_chip_detect-----------------------");
		printk("%s:Read driver IC ID = %X,%X,%X\n", __func__, tmp_data[3],
