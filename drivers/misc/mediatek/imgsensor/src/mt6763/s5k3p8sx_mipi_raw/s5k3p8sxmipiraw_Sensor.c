...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3P8SX_SENSOR_ID,      ////Sensor ID Value: 0x3010//record sensor id defined in Kd_imgsensor.h

	.checksum_value =0xB1F1B3CC,

	.pre = {
        .pclk = 560000000,              //record different mode's pclk
		.linelength = 10240,   //5120,	/* record different mode's linelength */
    .framelength = 1822,   //3643,         //record different mode's framelength
	...
	.i2c_addr_table = {0x21,0x5b,0x20,0x5a,0xff},         //record sensor support all write id addr, only supprt 4 must end with 0xff
};
...
{	{4640, 3488, 16, 16, 4608, 3456, 2304, 1728, 0000, 0000, 2304, 1728, 0, 0, 2304, 1728},	/* Preview */
	{4640, 3488, 16, 16, 4608, 3456, 2304, 1728, 0000, 0000, 2304, 1728, 0, 0, 2304, 1728},	/* capture */
	{4640, 3488, 16, 16, 4608, 3456, 2304, 1728, 0000, 0000, 2304, 1728, 0, 0, 2304, 1728},	/* video */
	{4640, 3488, 400, 664, 3840, 2160, 1280, 720, 0000, 0000, 1280, 720, 0, 0, 1280, 720},	/* hight speed video */
	{4640, 3488, 400, 664, 3840, 2160, 1920, 1080, 0000, 0000, 1920, 1080, 0, 0, 2304, 1728},/* slim video */
};
...
static void preview_setting(void)
{
  ...
  write_cmos_sensor(0x030E,  0x003C);
  ...
  write_cmos_sensor(0x301E,  0x0000);
  ...
  write_cmos_sensor(0x0340,  0x071E);
  ...
  write_cmos_sensor(0x36C4,  0xFFCD);
  write_cmos_sensor(0x36C6,  0xFFCD);
  write_cmos_sensor(0x36C8,  0xFFCD);
  write_cmos_sensor(0x36CA,  0xFFCD);
  write_cmos_sensor(0x36CC,  0xFFCD);
  write_cmos_sensor(0x36CE,  0xFFCD);
  write_cmos_sensor(0x36D0,  0xFFCD);
  write_cmos_sensor(0x36D2,  0xFFCD);
  write_cmos_sensor(0x36D4,  0xFFCD);
  write_cmos_sensor(0x36D6,  0xFFCD);
  write_cmos_sensor(0x36D8,  0xFFCD);
  write_cmos_sensor(0x36DA,  0xFFCD);
  write_cmos_sensor(0x36DC,  0xFFCD);
  write_cmos_sensor(0x36DE,  0xFFCD);
  write_cmos_sensor(0x36E0,  0xFFCD);
  write_cmos_sensor(0x36E2,  0xFFCD);
...
static void hs_video_setting(void)
{
  ...
  write_cmos_sensor(0x0340,  0x038F);
  write_cmos_sensor(0x602A,  0x1704);
  write_cmos_sensor(0x6F12,  0x8011);
  write_cmos_sensor(0x317A,  0x0045);
  write_cmos_sensor(0x31A4,  0x0102);
  write_cmos_sensor(0x36C4,  0xFFCD);
  write_cmos_sensor(0x36C6,  0xFFCD);
  write_cmos_sensor(0x36C8,  0xFFCD);
...
extern kal_bool s5k3p8sx_Read_Awb_Otp(kal_uint16 Awb_otp_start_addr, u16 Outdatalen,unsigned char * pOutputdata);


static kal_uint8 s5k3p8sx_awb_data[AWB_DATA_SIZE] = {0};

static void s5k3p8sx_apply_awb(void)
{
	...
	s5k3p8sx_Read_Awb_Otp(0x0006,AWB_DATA_SIZE,s5k3p8sx_awb_data);
	rg = (s5k3p8sx_awb_data[0] << 8) | s5k3p8sx_awb_data[1];
	bg = (s5k3p8sx_awb_data[2] << 8) | s5k3p8sx_awb_data[3];
	golden_rg = (s5k3p8sx_awb_data[4] << 8) | s5k3p8sx_awb_data[5];
	golden_bg = (s5k3p8sx_awb_data[6] << 8) | s5k3p8sx_awb_data[7];
	r_ratio = 512 * (golden_rg) /(rg);
	b_ratio = 512 * (golden_bg) /(bg);
	printk("###########pzp rg=%d bg=%d golden_rg=%d golden_bg=%d\n", rg, bg, golden_rg, golden_bg);
	if( !r_ratio || !b_ratio)
	{
		return;
	}
...
*
*************************************************************************/
/* zhengjiang.zhu@Koobee..Camera.Driver  2018/05/30  add  for Compatible module OTP */
static kal_uint16 get_module_id(kal_uint16 addr)
{
    kal_uint16 get_byte=0;
    char pusendcmd[2] = {(char)(addr >> 8) , (char)(addr & 0xFF) };
...
/* zhengjiang.zhu@Koobee..Camera.Driver  2018/05/30  end for Compatible module OTP */
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	//printk("fsd---get_imgsensor_id s5k3p8\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor_8(0x0000) << 8) | read_cmos_sensor_8(0x0001));
			year = get_module_id(0x0003);
			month = get_module_id(0x0004);
			printk("fsd---s5k3p8sx get_imgsensor_id id is 0x%x\n",*sensor_id);
			printk("read_0x0000=0x%x, 0x0001=0x%x,0x0000_0001=0x%x year=%d month=%d\n",read_cmos_sensor_8(0x0000),read_cmos_sensor_8(0x0001),read_cmos_sensor(0x0000),year,month);
			if (((*sensor_id ==imgsensor_info.sensor_id) && (year == 0x12) && (month < 0x04))) {
