...
/*prize  add  for camera esd by zhuzhengjiang    20190610-begin*/
extern void set_avdd_regulator(int status);
/*prize  add  for camera esd by zhuzhengjiang    20190610-end*/
	...
	char *pcurr_idx)
{
	...
	/*prize  add  for camera esd by zhuzhengjiang    20190610-begin*/
	if(sensor_idx ==0 && pwr_status == IMGSENSOR_HW_POWER_STATUS_ON)
	{
		set_avdd_regulator(1);
	}
	else if(sensor_idx ==0 && pwr_status == IMGSENSOR_HW_POWER_STATUS_OFF)
	{
