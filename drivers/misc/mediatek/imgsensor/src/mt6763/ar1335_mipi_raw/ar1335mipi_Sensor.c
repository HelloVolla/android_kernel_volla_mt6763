...
/******* Start : Add by zhouhongbiin 2017.07.29 ********/
/*
static void set_dummy(void);
static void hs_video_setting(void);
static void slim_video_setting(void);
*/
/******* End : Add by zhouhongbiin 2017.07.29 *********/

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static int OTP_KEY = 0;//0;

extern int Sunny_AR1335_read_OTP(void);
static imgsensor_info_struct imgsensor_info = {
	.sensor_id =AR1335_MIPI_SENSOR_ID,

	.checksum_value =0xffb1ec31,

	.pre = {
		.pclk = 512000000,                //record different mode's pclk   170309:375000000   170313:345000000
		.linelength = 5728,                //record different mode's linelength   4614
		.framelength = 3186,            //record different mode's framelength     2786
...
{
	{ 4224, 3136,     16,    16, 4208, 3120, 4208, 3120, 0000, 0000, 4208,  3120,      0,    0, 4208, 3120}, // Preview
...
#if 1
//static 
kal_uint16 Read_cmos_sensor_2byte(kal_uint32 addr)
{
	kal_uint16 get_byte=0;
	kal_uint16 tmp = 0;
	char pu_send_cmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (Read_cmos_sensor_2byte(0x0204)==0x001F) 
		write_cmos_sensor_2byte(0x3780, 0x0000);
	else	
		write_cmos_sensor_2byte(0x3780, 0x8000);
	
	LOG_INF("chenxing log-- 0x204 = 0x%x\n ", Read_cmos_sensor_2byte(0x0204));
...
extern kal_uint16 R_Gain;
extern kal_uint16 B_Gain;
extern kal_uint16 Gr_Gain;
extern kal_uint16 Gb_Gain;
...
static void sensor_init(void)
{
	...
	if(OTP_KEY)
	{
		write_cmos_sensor(0x3780,0x80); //0x80
