...
static imgsensor_info_struct imgsensor_info = {
	...
	#if defined(CONFIG_PROJECT_ODM_DM5_SUNNY)
...
{
	printk("hehuale ov5670 dummyline = %d, dummypixels = %d \n", imgsensor.dummy_line, imgsensor.dummy_pixel);
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
	...
	printk("hehuale ov5670 framerate = %d, min framelength should enable = %d\n", framerate,min_framelength_en);
...
//	kal_uint32 frame_length = 0;
	...
	printk("hehuale ov5670 Exit! shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);

	//printk("hehuale ov5670 frame_length = %d ", frame_length);
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain < BASEGAIN || gain > 8 * BASEGAIN) {
		printk("hehuale ov5670 Error gain setting");
	...
	spin_unlock(&imgsensor_drv_lock);
	printk("hehuale ov5670 gain = %d ,reg[0x366a]= %d, reg_gain = 0x%x\n ", gain, ChangeFlag, reg_gain);
...
{
	printk("hehuale ov5670 le:0x%x, se:0x%x, gain:0x%x\n",le,se,gain);
...
{
	printk("hehuale ov5670 image_mirror = %d\n", image_mirror);
	...
	switch (image_mirror) {
		...
		case IMAGE_HV_MIRROR:
			...
			break;
		default:
			printk("hehuale ov5670 Error image_mirror setting\n");
	}
...
{
	printk("hehuale ov5670 OV5670_Sensor_Init_2lane E\n");
	...
	write_cmos_sensor(0x0302, 0x78); //;69  840Mbps    78   960Mbps
...
{
	printk("hehuale ov5670  OV5670PreviewSetting_2lane enter\n");
...
{
	printk("hehuale ov5670 OV5670CaptureSetting_2lane enter! currefps:%d\n",currefps);
...
{
	printk("hehuale ov5670 normal_video_setting Enter! currefps:%d\n",currefps);
...
{
	printk("hehuale ov5670 hs_video_setting enter!\n");
...
{
	printk("hehuale ov5670 slim_video_setting enter!\n");
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("hehuale ov5670 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
			}
			printk("hehuale ov5670 Read sensor id fail, write id:0x%x id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
...
static kal_uint32 open(void)
{
	...
	printk("hehuale ov5670 MIPI 2LANE\n");
	printk("hehuale ov5670 preview 1280*960@30fps,864Mbps/lane; video 1280*960@30fps,864Mbps/lane; capture 5M@30fps,864Mbps/lane\n");
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {//PIP capture: 24fps for less than 13M, 20fps for 16M,15fps for 20M
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
            printk("hehuale ov5670 Warning: current_fps %d fps is not support, so use cap's setting: %d fps!\n",imgsensor.current_fps,imgsensor_info.cap.max_framerate/10);
...
static kal_uint32 get_info(MSDK_SCENARIO_ID_ENUM scenario_id,
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	printk("hehuale ov5670 scenario_id = %d\n", scenario_id);
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	switch (scenario_id) {
		...
		case MSDK_SCENARIO_ID_SLIM_VIDEO:
			...
			break;
		default:
			printk("hehuale ov5670 Error ScenarioId setting");
			preview(image_window, sensor_config_data);
...
{
	printk("hehuale ov5670 framerate = %d\n ", framerate);
...
{
	printk("hehuale ov5670 enable = %d, framerate = %d \n", enable, framerate);
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 set_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	printk("hehuale ov5670 scenario_id = %d, framerate = %d\n", scenario_id, framerate);
	...
	switch (scenario_id) {
		...
		default:  //coding with  preview scenario by default
			...
			spin_unlock(&imgsensor_drv_lock);
			set_dummy();
			printk("hehuale ov5670 error scenario_id = %d, we use preview scenario \n", scenario_id);
			break;
	}
	return ERROR_NONE;
}
...
{
	printk("hehuale ov5670 enable: %d\n", enable);
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	printk("hehuale ov5670 feature_id = %d\n", feature_id);
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
            printk("hehuale ov5670 feature_Control imgsensor.pclk = %d,imgsensor.current_fps = %d\n", imgsensor.pclk,imgsensor.current_fps);
		...
		case SENSOR_FEATURE_SET_FRAMERATE:
            printk("hehuale ov5670 current fps :%d\n", (UINT32)*feature_data);
			spin_lock(&imgsensor_drv_lock);
            imgsensor.current_fps = *feature_data;
			spin_unlock(&imgsensor_drv_lock);
			break;
		case SENSOR_FEATURE_SET_HDR:
            printk("hehuale ov5670 ihdr enable :%d\n", (BOOL)*feature_data);
			spin_lock(&imgsensor_drv_lock);
            imgsensor.ihdr_en = (BOOL)*feature_data;
			spin_unlock(&imgsensor_drv_lock);
			break;
		case SENSOR_FEATURE_GET_CROP_INFO:
            printk("hehuale ov5670 SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d\n", (UINT32)*feature_data);
		...
		case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
            printk("hehuale ov5670 SENSOR_SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",(UINT16)*feature_data,(UINT16)*(feature_data+1),(UINT16)*(feature_data+2));
