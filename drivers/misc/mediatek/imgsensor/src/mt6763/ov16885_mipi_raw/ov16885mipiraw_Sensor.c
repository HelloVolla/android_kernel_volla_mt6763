...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV16885_MIPI_SENSOR_ID,
...
{{ 4704, 3536,     32,   24, 4640, 3488, 2320, 1744,  8, 8,  2304, 1728,     0,    0, 2304, 1728}, // Preview 
 ...
 { 4704, 3536,     32,   24, 4640, 3488, 2320, 1744,  8, 8,  2304, 1728,     0,    0, 2304, 1728},  //custom1
 { 4704, 3536,     32,   24, 4640, 3488, 2320, 1744,  8, 8,  2304, 1728,     0,    0, 2304, 1728}};// custom2
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		printk("[get_imgsensor_id]ov16885mipi sensor i2c_write_id=0x%x.\n",imgsensor.i2c_write_id);
		do {	
			*sensor_id = (read_cmos_sensor_8(0x300A) << 16) | (read_cmos_sensor_8(0x300B)<<8) | read_cmos_sensor_8(0x300C);
			printk("[get_imgsensor_id]ov16885mipi sensor *sensor_id=0x%x.\n",*sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("[get_imgsensor_id]ov16885mipi i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				*sensor_id = OV16885_MIPI_SENSOR_ID;
				//preload_eeprom_data();	  
				return ERROR_NONE;
			}
			LOG_INF("[get_imgsensor_id]ov16885mipiRead sensor id fail, write id:0x%x ,sensor Id:0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
