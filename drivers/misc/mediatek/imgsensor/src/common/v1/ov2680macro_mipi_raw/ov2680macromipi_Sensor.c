...
#include "ov2680macromipi_Sensor.h"
...
/*according toov2680macro datasheet p53 image cropping*/
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x3820, 0xc0);// c4
	write_cmos_sensor(0x3821, 0x00);// c4
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id()+2;
			printk("ov2680macro get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);				
...
UINT32 OV2680MACROMIPISensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*      OV2680MACRO_MIPI_RAW_SensorInit      */
