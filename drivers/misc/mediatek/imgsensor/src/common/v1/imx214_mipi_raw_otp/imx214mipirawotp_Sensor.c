		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0016) << 8) | read_cmos_sensor(0x0017)) + 2;
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				write_cmos_sensor(0x0A02, 0x0); //Select Page
				write_cmos_sensor(0x0A00, 0x01); //Read ON
				
				otp_flag = read_cmos_sensor(0x0A05);
				LOG_INF("liaojie otp_flag=0x%x\n",otp_flag);
				if(otp_flag != 0x07)
				{
					LOG_INF("liaojie check fail\n");
					*sensor_id = 0xFFFFFFFF;
					return ERROR_SENSOR_CONNECT_FAIL;
				}
					LOG_INF("liaojie check ok\n");
				return ERROR_NONE;
			}
