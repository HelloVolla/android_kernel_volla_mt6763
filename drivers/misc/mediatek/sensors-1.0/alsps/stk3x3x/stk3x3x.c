...
*******************************************************************************/
#ifdef STK_TUNE0
#define STK_MAX_MIN_DIFF		100  //80 
#define STK_LT_N_CT				50  //30 90    
#define STK_HT_N_CT				67  //60 135  
#endif /* #ifdef STK_TUNE0 */

#define STK_H_PS				5000
#define STK_H_HT				380
#define STK_H_LT				350
...
int stk3x3x_get_ps_thd_h(struct i2c_client *client, u16 *data)
{
    struct stk3x3x_priv *obj = i2c_get_clientdata(client);
    int ret = 0;
    u8 buf[2];
...
int stk3x3x_get_ps_thd_l(struct i2c_client *client, u16 *data)
{
    struct stk3x3x_priv *obj = i2c_get_clientdata(client);
    int ret = 0;
    u8 buf[2];
...
static int stk3x3x_enable_als(struct i2c_client *client, int enable)
{
	...
	/* prize modified by lifenfen, change irq status with als onoff state, 20190904 begin */
	if (obj->hw.polling_mode_als == 0)
	{
		if (enable)
		{
			enable_irq(obj->irq);
			APS_LOG("%s: enable_irq\n", __func__);
		}
		else
		{
			disable_irq(obj->irq);
			APS_LOG("%s: disable_irq\n", __func__);
		}
	}
	/* prize modified by lifenfen, change irq status with als onoff state, 20190904 end */
...
static int stk3x3x_enable_ps(struct i2c_client *client, int enable, int validate_reg)
{
	...
	/* prize modified by lifenfen, change irq status with ps onoff state, 20190904 begin */
...
err_out:
#if defined(CONFIG_OF)
/* prize modified by lifenfen, enable irq when calibration end as disable irq when calibration begin, 20190905 begin */
#if 0
	disable_irq(obj->irq);
#else
	enable_irq(obj->irq);
#endif
/* prize modified by lifenfen, enable irq when calibration end as disable irq when calibration begin, 20190905 end */
...
static int stk_ps_tune_zero_func_fae(struct stk3x3x_priv *obj)
{
   ...
   // int err;
    ...
    obj->data_count++;
	if(obj->data_count > 10000){
		obj->data_count = 0;
	}else if((obj->data_count%20)==6){
		...
		APS_LOG("cur ps=%d, flag=0x%x, dis=%d\n", word_data, flag, obj->ps_distance_last);
		APS_LOG("psi_set=%d, psi=%d, smudge=%d\n", obj->psi_set, obj->psi, obj->ps_thd_update);
	}else if((obj->data_count%200)==9){
		ret = stk3x3x_get_ps_thd_h(obj->client, &high_thd);
        if (ret < 0)
        {
            APS_ERR("%s fail, err=0x%x", __func__, ret);
            return ret;
		...
		ret = stk3x3x_get_ps_thd_l(obj->client, &low_thd);
        if (ret < 0)
        {
            APS_ERR("%s fail, err=0x%x", __func__, ret);
            return ret;
        }  
		APS_LOG("cur ht=%d, lt=%d\n", high_thd, low_thd);
	}else if((obj->data_count%500)==8){
		show_allreg();
	}
...
int stk3x3x_setup_eint(struct i2c_client *client)
{
    ...
    return 0;
		...
		if (node)
		{
			...
			/* prize modified by lifenfen, set irq wake function to fix bug 79978, 20190904 begin */
			else
			{
				disable_irq(stk3x3x_obj->irq);
				enable_irq_wake(stk3x3x_obj->irq);
			}
			#if 0
			enable_irq(stk3x3x_obj->irq);
			#endif
			/* prize modified by lifenfen, set irq wake function to fix bug 79978, 20190904 end */
		}
		else
		{
			APS_ERR("null irq node!!\n");
...
static int stk3x3x_init_client(struct i2c_client *client)
{
	...
	*/
	...
	data = 0x00;
	err = stk3x3x_master_send(client, 0xF4, &data, 1);
	if (err < 0)
	{
		APS_ERR("write 0xfa = %d\n", err);
		return err;
...
static int stk3x3x_get_als_value(struct stk3x3x_priv *obj, u16 als)
{
	...
	APS_LOG("stk3x3x_get_als_value: %d\n", obj->hw.als_value[idx]);
	...
	if (!invalid){
				...
				APS_LOG("ALS: %d [%d, %d] => %d [%d, %d]  value_diff = %d level_diff = %d \n", als, level_low, level_high, value, value_low, value_high,value_diff,level_diff);		
					}
					return value;
				}
	else
...
#if 0 //prize-chj-20190703 Use a simple light sensitivity factor instead of a device tree light level start
		else
		{
			*value = stk3x3x_get_als_value(obj, obj->als_last);
		}
#endif
            *value = obj->als * 36 / 100;
 //prize-chj-20190703 Use a simple light sensitivity factor instead of a device tree light level end
		*status = SENSOR_STATUS_ACCURACY_MEDIUM;
	}
...
static int stk3x3x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (node == NULL)
		printk("get compatible node fail!\n");
	else {
		...
		if (ret) {
			...
			printk("get stk3x3x dts info ok! i2c_num:%d\n", obj->hw.i2c_num);
	}
...
#if defined(CONFIG_PRIZE_HARDWARE_INFO)
	strcpy(current_alsps_info.chip,"STK3311");
...
static int __init stk3x3x_init(void)
{
	...
	if (node == NULL)
		...
		if (ret) {
			...
			printk("get dts info ok! i2c_num:%d\n", hw->i2c_num);
	}
#endif
	APS_FUN();
