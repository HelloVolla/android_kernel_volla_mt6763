...
int stk3x3x_read_als(struct i2c_client *client, u16 *data)
{
	...
	ret = stk3x3x_master_recv(obj->slave_client, obj->addr.data1_als, buf, 0x2);
...
int stk3x3x_write_als(struct i2c_client *client, u8 data)
{
	struct stk3x3x_priv *obj = i2c_get_clientdata(client);
	int ret = 0;
	ret = stk3x3x_master_send(obj->slave_client, obj->addr.alsctrl, &data, 1);
...
int stk3x3x_slave_write_state(struct i2c_client *client, u8 data)
{
	struct stk3x3x_priv *obj = i2c_get_clientdata(client);
	int ret = 0;
	ret = stk3x3x_master_send(obj->slave_client, obj->addr.state, &data, 1);
...
int stk3x3x_write_als_high_thd(struct i2c_client *client, u16 thd)
{
	...
	buf[0] = (u8)((0xFF00 & thd) >> 8);
	buf[1] = (u8)(0x00FF & thd);
	ret = stk3x3x_master_send(obj->slave_client, obj->addr.thdh1_als, &buf[0], 1);
	...
	ret = stk3x3x_master_send(obj->slave_client, obj->addr.thdh2_als, &(buf[1]), 1);
...
int stk3x3x_write_als_low_thd(struct i2c_client *client, u16 thd)
{
	...
	buf[0] = (u8)((0xFF00 & thd) >> 8);
	buf[1] = (u8)(0x00FF & thd);
	ret = stk3x3x_master_send(obj->slave_client, obj->addr.thdl1_als, &buf[0], 1);
	...
	ret = stk3x3x_master_send(obj->slave_client, obj->addr.thdl2_als, &(buf[1]), 1);
...
static int slave_show_allreg(void)
{
	int ret = 0;
	u8 reg[10] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0xDB, 0xFA, 0x3E, 0x3F};
	u8 rbuf;
	int cnt;
	...
	for(cnt = 0; cnt < 10; cnt++){
		rbuf = 0x00;
		ret = stk3x3x_master_recv(stk3x3x_obj->slave_client, reg[cnt], &rbuf, 1);
		if (ret < 0)
		{
			APS_DBG("error: %d\n", ret);
		}else{
...
static int stk3x3x_enable_als(struct i2c_client *client, int enable)
{
	...
	err = stk3x3x_slave_write_state(client, cur);

	if (err < 0)
		return err;
	else
		atomic_set(&obj->slave_state_val, cur);
	...
	buf = 0x30;
	stk3x3x_master_send(obj->slave_client, 0x01, &buf, 1);
//prize-Modifying the base noise value of ALS is about 250-pengzhipeng-20190620-start
	buf = 0x22;
//prize-Modifying the base noise value of ALS is about 250-pengzhipeng-20190620-end
	stk3x3x_master_send(obj->slave_client, 0x02, &buf, 1);

	buf = 0xA0;
	stk3x3x_master_send(obj->slave_client, 0x03, &buf, 1);

	buf = 0x00;
	stk3x3x_master_send(obj->slave_client, 0x04, &buf, 1);

	buf = 0x1F;
	stk3x3x_master_send(obj->slave_client, 0x05, &buf, 1);				
#ifdef STK_PS_DEBUG
	APS_LOG("%s:show all reg\n", __FUNCTION__);
	slave_show_allreg(); //for debug
#endif
	return 0;
}
/*----------------------------------------------------------------------------*/
...
//prize-Optimized Psensor drum drop address difference of about 2000 causes failure-pengzhipeng-20190628-start
	/*
	...
	*/
//prize-Optimized Psensor drum drop address difference of about 2000 causes failure-pengzhipeng-20190628-end
	ret = stk3x3x_ps_val();
	...
	if (diff > obj->stk_max_min_diff)
	{
		...
		if ((atomic_read(&obj->ps_high_thd_val) > (obj->ps_high_thd_boot + 500)) && (obj->boot_cali == 1))
		{
			...
			*/
			//prize-Optimized Psensor drum drop address difference of about 2000 causes failure-pengzhipeng-20190628-end
		//	APS_LOG("%s: current HT=%d, boot HT=%d\n", __func__,atomic_read(&obj->ps_high_thd_val),atomic_read(&obj->ps_high_thd_boot));
			APS_LOG("%s: current HT=%d, boot LT=%d\n", __func__,atomic_read(&obj->ps_high_thd_val),obj->ps_high_thd_boot);			
		}
		else
		{
			if ((atomic_read(&obj->ps_high_thd_val) + 100) < (obj->ps_high_thd_boot))
...
int stk3x3x_setup_eint(struct i2c_client *client)
{
	...
	if (node)
	{
		...
		stk3x3x_obj->irq = irq_of_parse_and_map(node, 0);
...
static int stk3x3x_init_client(struct i2c_client *client)
{
	...
	*/
	...
	//A GAIN default
	//stk3x3x_master_send(client, 0xDB, 0x15, 1);
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x3x_slave_show_send(struct device_driver *ddri, char *buf)
{
	return 0;
}
/*----------------------------------------------------------------------------*/
static ssize_t stk3x3x_slave_store_send(struct device_driver *ddri, const char *buf, size_t count)
{
	int addr, cmd;
	u8 dat;
	...
	dat = (u8)cmd;
	APS_LOG("send(%02X, %02X) = %d\n", addr, cmd,
		stk3x3x_master_send(stk3x3x_obj->slave_client,
		(u16)addr, &dat, sizeof(dat)));
	return count;
}
/*----------------------------------------------------------------------------*/
static ssize_t stk3x3x_slave_show_recv(struct device_driver *ddri, char *buf)
{
	if (!stk3x3x_obj)
	{
		APS_ERR("stk3x3x_obj is null!!\n");
...
/*----------------------------------------------------------------------------*/
static ssize_t stk3x3x_slave_store_recv(struct device_driver *ddri, const char *buf, size_t count)
{
	int addr;
	u8 dat;
	...
	APS_LOG("recv(%02X) = %d, 0x%02X\n", addr,
		stk3x3x_master_recv(stk3x3x_obj->slave_client, (u16)addr, (char*)&dat, sizeof(dat)), dat);
	atomic_set(&stk3x3x_obj->recv_reg, dat);
	return count;
}
/*----------------------------------------------------------------------------*/
...
static DRIVER_ATTR(slave_send, S_IWUSR | S_IRUGO, stk3x3x_slave_show_send, stk3x3x_slave_store_send);
static DRIVER_ATTR(slave_recv, S_IWUSR | S_IRUGO, stk3x3x_slave_show_recv, stk3x3x_slave_store_recv);
...
static struct driver_attribute *stk3x3x_attr_list[] =
{
	...
	&driver_attr_send,
	&driver_attr_recv,
	&driver_attr_slave_send,
	&driver_attr_slave_recv,	
	&driver_attr_allreg,
	&driver_attr_reg,
...
static int als_get_data(int* value, int* status)
{
	...
	obj->als_debug_count ++;
	if(obj->als_debug_count > 10000){
		obj->als_debug_count = 0;
	}else if((obj->als_debug_count%10)==5){
		APS_ERR("ALS: als=%d, lux=%d\n", obj->als, (*value));
	}else if((obj->als_debug_count%20)==7){
		#ifdef STK_PS_DEBUG
...
static int stk3x3x_init_reg(struct i2c_client *client)
{
	...
	atomic_set(&obj->slave_state_val, 0);

	i2c_config_reg = STK_PS_PRS1 | STK_PS_GAIN8 | STK_PS_IT200;
	atomic_set(&obj->psctrl_val, i2c_config_reg);

	i2c_config_reg = STK_ALS_PRS1 | STK_ALS_GAIN64 | STK_ALS_IT25;
...
static int stk3x3x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(!(slave_client = kzalloc(sizeof(struct i2c_client), GFP_KERNEL))) 
	{
		err = -ENOMEM;
		goto exit;
	}
	memset(slave_client, 0, sizeof(struct i2c_client));
	...
	memcpy(slave_client, stk3x3x_i2c_client, sizeof(struct i2c_client));
	APS_LOG("before slave client addr:0x%x\n", slave_client->addr);
	slave_client->addr = 0x57; //v2000
	obj->slave_client = slave_client;
