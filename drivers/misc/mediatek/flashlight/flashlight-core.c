 ...
 * File operations
 *****************************************************************************/
 #if defined(CONFIG_PRIZE_ISINK_SUB_FLASHLIGHT)||defined(CONFIG_MTK_FLASHLIGHT_GPIO)
 u8 prize_sub_flashlight_precapture=0;
 #endif
		...
		struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_SET_DUTY:
		...
		mutex_unlock(&fl_mutex);
	 #if defined(CONFIG_PRIZE_ISINK_SUB_FLASHLIGHT)||defined(CONFIG_MTK_FLASHLIGHT_GPIO)
		if((type==1)&&(fl_arg.arg>0 && fl_arg.arg<6 ))
		{
			prize_sub_flashlight_precapture=1;
		}
	 #endif
		break;
	...
	case FLASH_IOC_SET_ONOFF:
	 #if defined(CONFIG_PRIZE_ISINK_SUB_FLASHLIGHT)||defined(CONFIG_MTK_FLASHLIGHT_GPIO)
		printk("FLASH_IOC_SET_ONOFF(%d,%d,%d),prize_sub_flashlight_precapture=%d\n",
				type, ct, part,prize_sub_flashlight_precapture);
		if((prize_sub_flashlight_precapture==1)&&(fl_arg.arg==0))
		{
			ret=0;
			prize_sub_flashlight_precapture=0;
			break;
		}
	 #else
...
{
	struct flashlight_dev *fdev;
	//struct flashlight_dev *fdev2;
	struct flashlight_dev_arg fl_dev_arg;
	int type, ct, part,part_id;
	int ret;
	flashlight_state = *buf - '0';
	pr_debug("flashlight_torch_store entry  flashlight_state=%d\n",flashlight_state);

	/* find flashlight device */
	mutex_lock(&fl_mutex);
	fdev = flashlight_find_dev_by_index(
			flashlight_get_type_index(1),
			flashlight_get_ct_index(1));
       #if 0
	fdev2 = flashlight_find_dev_by_index(
			flashlight_get_type_index(1),
			flashlight_get_ct_index(2));
	#endif
	mutex_unlock(&fl_mutex);
	if (!fdev) {
		pr_info("Find no flashlight device\n");
	...
	if (!fdev2) {
       ...
       #endif
	/* led1 setup flash dev arguments */
	//fl_dev_arg.arg = fl_arg.arg;
	fl_dev_arg.channel = fdev->dev_id.channel;
	type = fdev->dev_id.type;
	ct = fdev->dev_id.ct;
	...
	if (fdev->ops) {
		...
		//FLASH_IOC_SET_DUTY
		fl_dev_arg.arg = FLASHLIGHT_TORCH_DUTY;
		if (fdev->ops->flashlight_ioctl(FLASH_IOC_SET_DUTY,
				(unsigned long)&fl_dev_arg)) {
			pr_err("Failed to set level\n");
			return -EFAULT;
	...
	#if 0
	/* led2 setup flash dev arguments */
	//fl_dev_arg.arg = fl_arg.arg;
	...
	if (fdev2->ops) {
		...
		//FLASH_IOC_SET_DUTY
		fl_dev_arg.arg = FLASHLIGHT_TORCH_DUTY;
		if (fdev2->ops->flashlight_ioctl(FLASH_IOC_SET_DUTY,
				(unsigned long)&fl_dev_arg)) {
			pr_err("Failed to set level\n");
			return -EFAULT;
		}
		...
		//FLASH_IOC_SET_ONOFF
		fl_dev_arg.arg = flashlight_state;
		if (fdev2->ops->flashlight_ioctl(FLASH_IOC_SET_ONOFF,
				(unsigned long)&fl_dev_arg)) {
			pr_err("Failed to set on/off\n");
			return -EFAULT;
		}
