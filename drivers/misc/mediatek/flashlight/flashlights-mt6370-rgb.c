...
#ifndef MT6370_RGB_DTNAME
#define MT6370_RGB_DTNAME "mediatek,flashlights_mt6370_rgb"
#endif

#define MT6370_NAME "flashlights-mt6370-rgb"
...
{
	int ret = 0;
	enum flashlight_mode mode = FLASHLIGHT_MODE_SUB_TORCH;
printk("mt6370_enable mode:%d\n",mode);
	...
	if ((mt6370_en_ch1 == MT6370_ENABLE_FLASH)
		...
		mode = FLASHLIGHT_MODE_SUB_TORCH;
	...
	if (mt6370_en_ch1)
		...
		ret |= flashlight_set_mode(
				flashlight_dev_ch1, FLASHLIGHT_MODE_SUB_OFF);
	if (mt6370_en_ch2)
		ret |= flashlight_set_mode(
				flashlight_dev_ch2, mode);
	else
		ret |= flashlight_set_mode(
				flashlight_dev_ch2, FLASHLIGHT_MODE_SUB_OFF);
...
static int mt6370_disable(void)
{
	...
	ret |= flashlight_set_mode(flashlight_dev_ch1, FLASHLIGHT_MODE_SUB_OFF);
	ret |= flashlight_set_mode(flashlight_dev_ch2, FLASHLIGHT_MODE_SUB_OFF);
...
{
	ktime_t ktime;
printk("mt6370_operate channel:%d enable:%d\n",channel, enable);
  ...
  //mt6370_en_ch2 = mt6370_en_ch1;//prize wyq 20191009 remove
	/* decouple mode */
...
printk("mt6370_operate mt6370_en_ch2:%d mt6370_en_ch1:%d\n",mt6370_en_ch2, mt6370_en_ch1);
...
static int mt6370_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	case FLASH_IOC_SET_ONOFF:
		printk("hushilun: FLASH_IOC_SET_ONOFF(%d): %d\n",
				channel, (int)fl_arg->arg);
...
//	int i;
	...
	if (!flashlight_dev_ch1) {
		printk("Failed to get ht flashlight device.\n");
		return -EFAULT;
	}
	else
	{
		printk("hushilun: flashlight_dev_ch1 ok\n");
	}
	flashlight_dev_ch2 = find_flashlight_by_name(RT_FLED_DEVICE_CH2);
	if (!flashlight_dev_ch2) {
		printk("Failed to get lt flashlight device.\n");
		return -EFAULT;
	}
	else
	{
		printk("hushilun: flashlight_dev_ch2 ok\n");
	}
	/* setup strobe mode timeout */
	if (flashlight_set_strobe_timeout(flashlight_dev_ch1,
				MT6370_HW_TIMEOUT, MT6370_HW_TIMEOUT + 200) < 0)
		printk("Failed to set strobe timeout.\n");
printk("hushilun: flashlight_dev_ch test 2\n");
	...
	printk("hushilun: flashlight_dev_ch test 3\n");
	if (!flashlight_charger_consumer) {
		pr_err("Failed to get charger manager.\n");
		return -EFAULT;
	}
	printk("hushilun: flashlight_dev_ch test 4\n");
...
static int __init flashlight_mt6370_rgb_init(void)
{
	int ret;
...
static void __exit flashlight_mt6370_rgb_exit(void)
{
	pr_debug("Exit start.\n");
...
late_initcall(flashlight_mt6370_rgb_init);
module_exit(flashlight_mt6370_rgb_exit);
