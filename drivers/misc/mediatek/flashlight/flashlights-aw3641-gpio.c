...
static int __init flashlight_aw3641_init(void)
{
	...
	pr_err("lsw_aw3641 Init start.\n");

#ifndef CONFIG_OF
	ret = platform_device_register(&aw3641_gpio_platform_device);
	if (ret) {
		pr_err("lsw_aw3641 Failed to register platform device\n");
		return ret;
	}
#endif
	...
	if (ret) {
		pr_err("lsw_aw3641 Failed to register platform driver\n");
		return ret;
	}

	pr_err("lsw_aw3641 Init done.\n");
