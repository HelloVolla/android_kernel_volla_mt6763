...
static struct aw2015_led *aw2015_led_dev = NULL;
static bool charge_mode_switch = false;
...
static void aw2015_brightness_work(struct work_struct *work)
{
	...
	if(charge_mode_switch){
		...
		aw2015_read(led, AW2015_REG_GCR, &val);
		if (!(val&AW2015_LED_CHIP_ENABLE_MASK)) {
			aw2015_write(led, AW2015_REG_GCR, AW2015_LED_CHIP_ENABLE_MASK);
...
//prize lsw add for leds flicker mode start
	if(charge_mode_switch){
		if(blinking > 0){
			//msleep(1000);
			aw2015_read(led, AW2015_REG_GCR, &val);
...
//prize lsw add for leds flicker mode end

}
//prize lsw add for leds flicker mode start	
static void aw2015_led_flicker_set(struct aw2015_led *led, unsigned long blinking)
{
	u8 val = 0;
	
	u8 prize_rise_time_ms = 0;
	u8 prize_hold_time_ms = 5;
	u8 prize_fall_time_ms = 0;
	u8 prize_off_time_ms  = 7;
	...
	if (blinking > 0) {
		...
		aw2015_write(led, AW2015_REG_PWM1 + led->id, AW2015_LED_BREATHE_PWM_MASK);
		aw2015_write(led, AW2015_REG_PAT1_T1 + led->id*5, 
					(prize_rise_time_ms << 4 | prize_hold_time_ms));
		aw2015_write(led, AW2015_REG_PAT1_T2 + led->id*5, 
					(prize_fall_time_ms << 4 | prize_off_time_ms));
...
//prize lsw add for leds flicker mode end	
void aw2015_charge_mode(int on_off)
{
	//struct aw2015_led *led = container_of(cdev, struct aw2015_led, cdev);
	if(on_off){
		aw2015_led_dev->cdev.brightness = 255;
		charge_mode_switch = true;
	}else{
		charge_mode_switch = false;
		aw2015_led_dev->cdev.brightness = 0;
	}

	schedule_work(&aw2015_led_dev->brightness_work);
}
...
//prize lsw add for leds flicker mode start	
static ssize_t aw2015_store_flicker(struct device *dev,
			     struct device_attribute *attr,
			     const char *buf, size_t len)
{
	unsigned long blinking = 0;
	...
	if (ret)
		return ret;
	mutex_lock(&led->pdata->led->lock);
	aw2015_led_flicker_set(led, blinking);
	mutex_unlock(&led->pdata->led->lock);
...
//prize lsw add for leds flicker mode start	
static DEVICE_ATTR(flicker, 0664, NULL, aw2015_store_flicker);
//prize lsw add for leds flicker mode end	
