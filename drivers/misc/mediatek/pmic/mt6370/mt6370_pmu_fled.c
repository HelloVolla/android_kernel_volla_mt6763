...
extern void mt6370_pmu_led_bright_set_ext(struct mt6370_pmu_chip *chip,int bright);
extern int subcamera_enable(void);
extern int subcamera_disable(void);
#endif
					...
					enum flashlight_mode mode)
{
	struct mt6370_pmu_fled_data *fi = (struct mt6370_pmu_fled_data *)info;
	int ret = 0;
printk("mt6370_fled_set_mode mode:%d\n", mode);
	switch (mode) {
	...
	case FLASHLIGHT_MODE_OFF:
		...
		break;
#if defined(CONFIG_PRIZE_ISINK_SUB_FLASHLIGHT)||defined(CONFIG_MTK_FLASHLIGHT_GPIO)
	case FLASHLIGHT_MODE_SUB_TORCH:
	#if defined(CONFIG_PRIZE_ISINK_SUB_FLASHLIGHT)
		mt6370_pmu_led_bright_set_ext(fi->chip,255);
	#else
		subcamera_enable();
	#endif
		break;
	case FLASHLIGHT_MODE_SUB_OFF:
	#if defined(CONFIG_PRIZE_ISINK_SUB_FLASHLIGHT)
		mt6370_pmu_led_bright_set_ext(fi->chip,0);
	#else
		subcamera_disable();
	#endif
		break;
