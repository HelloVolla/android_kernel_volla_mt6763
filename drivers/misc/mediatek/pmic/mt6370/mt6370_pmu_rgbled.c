...
#if defined(CONFIG_PRIZE_ISINK_SUB_FLASHLIGHT)
void mt6370_pmu_led_bright_set_ext(struct mt6370_pmu_chip *chip,int bright)
{

	int led_index = 0;
	uint8_t reg_addr = 0, reg_mask = 0x7, en_mask = 0;
	bool need_enable_timer = true;
	int ret = 0;

	printk("hushilun: mt6370_pmu_led_bright_set_ext bright:%d\n", bright);
	for(led_index=MT6370_PMU_LED1;led_index<=MT6370_PMU_LED3;led_index++)
	{
		switch (led_index) {
		case MT6370_PMU_LED1:
			reg_addr = MT6370_PMU_REG_RGB1ISINK;
		...
		if(bright>0)
			...
			bright=0;
		ret=mt6370_pmu_reg_update_bits(chip,
						reg_addr, reg_mask, bright);
	
		if (ret < 0) {
			printk("update brightness fail\n");
			return;
		}
		#if 0
		if (!bright)
		...
		en_mask=0xE0;
			ret =mt6370_pmu_reg_update_bits(chip, MT6370_PMU_REG_RGBEN,en_mask,(bright > 0) ? en_mask : ~en_mask);
		if (ret < 0)
				printk("update enable bit fail\n");
